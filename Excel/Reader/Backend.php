...
abstract class sb_Excel_Reader_Backend {
	...
	public function read($sFileName) {
		...
		$this->num_big_block_depot_blocks = $this->get_int_4d($this->ole_data, self::NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
		$this->sbd_start_block = $this->get_int_4d($this->ole_data, self::SMALL_BLOCK_DEPOT_BLOCK_POS);
		$this->root_start_bloc = $this->get_int_4d($this->ole_data, self::ROOT_START_BLOCK_POS);
		$this->extension_block = $this->get_int_4d($this->ole_data, self::EXTENSION_BLOCK_POS);
		$this->num_extension_blocks = $this->get_int_4d($this->ole_data, self::NUM_EXTENSION_BLOCK_POS);
		...
		for ($i = 0; $i < $bbdBlocks; $i++) {
			$bigBlockDepotBlocks[$i] = $this->get_int_4d($this->ole_data, $pos);
			$pos += 4;
		}
		...
		for ($j = 0; $j < $this->num_extension_blocks; $j++) {
			...
			if ($bbdBlocks < $this->num_big_block_depot_blocks) {
				$this->extension_block = $this->get_int_4d($this->ole_data, $pos);
			}
		}
		...
		for ($i = 0; $i < $this->num_big_block_depot_blocks; $i++) {
			...
			for ($j = 0; $j < self::BIG_BLOCK_SIZE / 4; $j++) {
				$this->big_block_chain[$index] = $this->get_int_4d($this->ole_data, $pos);
				$pos += 4;
				$index++;
			}
		}
		...
		while ($sbdBlock != -2) {
			...
			for ($j = 0; $j < self::BIG_BLOCK_SIZE / 4; $j++) {
				$this->small_block_chain[$index] = $this->get_int_4d($this->ole_data, $pos);
				$pos += 4;
				$index++;
			}
