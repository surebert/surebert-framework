...
class sb_Excel_ReaderOLE {
	...
	public function read($sFileName) {
		...
		$this->num_big_block_depot_blocks = $this->get_int_4d($this->data, self::NUM_BIG_BLOCK_DEPOT_BLOCKS_POS);
		$this->sbd_start_block = $this->get_int_4d($this->data, self::SMALL_BLOCK_DEPOT_BLOCK_POS);
		$this->root_start_bloc = $this->get_int_4d($this->data, self::ROOT_START_BLOCK_POS);
		$this->extension_block = $this->get_int_4d($this->data, self::EXTENSION_BLOCK_POS);
		$this->num_extension_blocks = $this->get_int_4d($this->data, self::NUM_EXTENSION_BLOCK_POS);
		...
		for ($i = 0; $i < $bbdBlocks; $i++) {
			$bigBlockDepotBlocks[$i] = $this->get_int_4d($this->data, $pos);
			$pos += 4;
		}
		...
		for ($j = 0; $j < $this->num_extension_blocks; $j++) {
			...
			if ($bbdBlocks < $this->num_big_block_depot_blocks) {
				$this->extension_block = $this->get_int_4d($this->data, $pos);
			}
		}
		...
		for ($i = 0; $i < $this->num_big_block_depot_blocks; $i++) {
			...
			for ($j = 0; $j < self::BIG_BLOCK_SIZE / 4; $j++) {
				$this->big_block_chain[$index] = $this->get_int_4d($this->data, $pos);
				$pos += 4;
				$index++;
			}
		}
		...
		while ($sbdBlock != -2) {
			...
			for ($j = 0; $j < self::BIG_BLOCK_SIZE / 4; $j++) {
				$this->small_block_chain[$index] = $this->get_int_4d($this->data, $pos);
				$pos += 4;
				$index++;
			}
	...
	protected function read_property_sets() {
		...
		while ($offset < strlen($this->entry)) {
			...
			$startBlock = $this->get_int_4d($d, self::START_BLOCK_POS);
			$size = $this->get_int_4d($d, self::SIZE_POS);
			$name = '';
...
class sb_Excel_ReaderBackend {
	...
	protected function read($sFileName) {
		...
		$this->data = $this->ole->get_workbook();
		$this->parse();
	}
		...
		while ($code != self::SPREADSHEET_EXCEL_READER_TYPE_EOF) {
			...
			switch ($code) {
				case self::SPREADSHEET_EXCEL_READER_TYPE_SST:
					...
					$uniqueStrings = $this->get_int_4d($data, $spos+4);
					$spos += 8;
					for ($i = 0; $i < $uniqueStrings; $i++) {
						...
						if ($extendedString) {
							// Read in cchExtRst
							$extendedRunLength = $this->get_int_4d($data, $spos);
							$spos += 4;
						}
						...
						$retstr = ($asciiEncoding) ? $retstr : $this->encode_UTF_16($retstr);
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_FONT:
						...
						$this->fontRecords[] = array(
								...
								'bold' => ($weight==700),
								'font' => $font,
								'raw' => $this->dump_hex_data($data, $pos+3, $length)
								);
					    break;
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_BOUNDSHEET:
						$rec_offset = $this->get_int_4d($data, $pos+4);
						...
						if ($version == self::SPREADSHEET_EXCEL_READER_BIFF8){
							...
							if ($chartype == 0){
								...
								$rec_name	= $this->encode_UTF_16(substr($data, $pos+12, $rec_length*2));
							}
	...
	protected function parse_sheet($spos) {
		...
		while($cont) {
			...
			switch ($code) {
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_RK2:
					...
					$rknum = $this->get_int_4d($data, $spos + 6);
					$numValue = $this->get_IEEE754($rknum);
					$info = $this->get_cell_details($spos,$numValue,$column);
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_LABELSST:
					...
					$index  = $this->get_int_4d($data, $spos + 6);
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_MULRK:
					...
					$tmppos = $spos+4;
					for ($i = 0; $i < $columns; $i++) {
						$numValue = $this->get_IEEE754($this->get_int_4d($data, $tmppos + 2));
						$info = $this->get_cell_details($tmppos-4,$numValue,$colFirst + $i + 1);
						$tmppos += 6;
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_NUMBER:
					...
					if ($this->is_date($spos)) {
						$numValue = $tmp['double'];
					}
					else {
						$numValue = $this->create_number($spos);
					}
					$info = $this->get_cell_details($spos,$numValue,$column);
					$this->addcell($row, $column, $info['string'], $info);
					break;
                ...
                case self::SPREADSHEET_EXCEL_READER_TYPE_STRING:
					...
					if ($version == self::SPREADSHEET_EXCEL_READER_BIFF8){
						...
						if ($extendedString) {
							// Read in cchExtRst
							$extendedRunLength =$this->get_int_4d($this->data, $xpos);
							$xpos += 4;
						}
						$len = ($asciiEncoding)?$numChars : $numChars*2;
				...
				case self::SPREADSHEET_EXCEL_READER_TYPE_HYPER:
					...
					if (($flags & 1) > 0 ) {   // is a type we understand
						...
						$ulink = $this->read_16_bit_string($this->data, $spos + $uloc + 20);
						if ($udesc == "") {
							$udesc = $ulink;
						}
					}
					$linkdata['desc'] = $udesc;
					$linkdata['link'] = $this->encode_UTF_16($ulink);
					for ($r=$row; $r<=$row2; $r++) {
		...
		protected function get_cell_details($spos,$numValue,$column) {
			...
			if ($type == 'date') {
				...
				$dateinfo = $this->gm_get_date($utcValue);
...
/**
 * Public API for reading Excel 2003 files
 */
class sb_Excel_Reader extends sb_Excel_ReaderBackend{
	 ...
	 * @param <type> $store_extended_info
	...
	public function __construct($file='', $store_extended_info=true, $outputEncoding='') {
		$this->ole = new sb_Excel_ReaderOLE();
