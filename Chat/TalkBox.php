...
class sb_Chat_TalkBox{
	...
	public function create_room($room){
			...
			$sql="INSERT INTO sb_TalkBox_master (name, created, last_visited, last_post) VALUES ('".$room."', NOW(), NOW(), NOW());";
	...
	public function insert(sb_Chat_Line $line){
		...
		$line->message = sb_Text_Bling::strip_all($line->message);
		
		$line->message = sb_Text_Bling::typo_fix($line->message);
		
		$sql = "INSERT INTO sb_TalkBox_".$this->room." ( uname, tstamp, message, ip) VALUES ( :uname, NOW(), :message, INET_ATON(:ip))";
	...
	public function display($id, $dir ="up", $limit=10){
		...
		$sql="SELECT id, DATE_FORMAT(tstamp,'%m/%d %h:%i') AS t, uname AS u, message AS m FROM sb_TalkBox_".$this->room."_mem WHERE id ".$ltgt." :id ";
