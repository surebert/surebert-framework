 ...
 *
 * @Author: Paul Visco
 * @Version: 1.31 4/17/2008 02/04/2009
 *<code>
App::$logger = new sb_Logger('audit,files,debug');
App::$logger->set_agent("\t".App::$user->uname."\t".App::$user->roswell_id."\t".Gateway::$remote_addr);
 ...
 *
 */
class sb_Logger_FileSystem extends sb_Logger{
	...
	*/
	public function __construct($log_types){
		...
		$this->_agent_str = Gateway::$remote_addr;
	...
	public function __call($log_type, $arguments){
		...
		if(in_array($log_type, $this->_enabled_logs)){
			$data = $arguments[0];
