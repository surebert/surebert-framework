<?php
/**
 * Used to plot simple point and line graphs.  Requires sb_RangeMapper
 * @Author Paul Visco
...
class sb_Graph_Point{
	...
	private function allocate_colors(){
		...
		$this->ink['point'] = imagecolorallocate ($this->im,0xff,0x00,0x00);
		$this->ink['point'] = imagecolorallocate ($this->im,0x44,0x44,0x44);
	}
	...
	private function map_y_value($value){
		...
		$rangeMapper = new sb_RangeMapper(Array(30, $this->graph_height), Array($this->min, $this->max));
	...
	private function draw_y_axis(){
		...
		if(!isset($this->y_axis_legend_increment)){
			$increment = round(($max-$min)/$this->total_values);
		} else {
	...
	private function plot_value($y){
		$rangeMapper = new sb_RangeMapper(Array($this->axis_offset, $this->graph_height-$this->axis_offset), Array($this->max, $this->min));
