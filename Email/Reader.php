...
class sb_Email_Reader{
	 ...
	 *
	 * @param string $address e.g. paul@example.com
	...
	public function fetch_messages(){
		...
		for($i=1;$i<$this->email_count+1;$i++){
			...
			$this->parse_header_info($email, imap_headerinfo($this->inbox, $i));
			...
			if(isset($structure->parts)){
				...
				foreach($structure->parts as $part){
					...
					//get type 0 = text, 1 = multipart, 2 = message, 3 = application, 4 = audio, 5= image, 6= video, 7 = other
					$type = $part->type;
					...
					if($type ==1 && isset($part->parts)){
						...
						$this->examine_part($email, $part, $part_id);
					}
