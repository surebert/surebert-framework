...
/**
 * Simple wrapper used to send authenticated email through exchange without having to use soap client
 */
class sb_Email_Exchange
{
     ...
     * 
     * <code>
      $mail = new sb_Email_Exchange("someaddress@gmail.com", "hello < world", "hey there");
     ...
     * @param string $passwd The password
     */
    public function setLogger(sb_Logger_Base $logger)
    {
        $this->logger = $logger;
    }
...
foreach($this->mail_headers as $k=>$val){
        ...
        if($this->logger){
            if($sent){
                $this->logger->sb_email_exchange_sent($xml);
            } else {
                $this->logger->sb_email_exchange_error($xml);
            }
        }
